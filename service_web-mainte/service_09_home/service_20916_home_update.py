...
from admin_app.dao.dao_m_white_paper import DaoTWhitePaper
...
class HomeUpdateService(ServiceMain):
    ...
    dao_m_white_paper = DaoTWhitePaper()
    ...
    def bizProcess(self,position):
        ...
        white_paper_table = self.mapping(DtoHomeupdate.DtoTWhitePaper,self.dao_m_white_paper.selectWhitePaper())
        dto_home_update = DtoHomeupdate(banner_table,white_paper_table)
        return self.unpack(dto_home_update)

    '''更新プロセス'''
    def homeUpdateProcess(self,position,title,details,external_site,white_paper_num,white_paper,is_checked,request):
        # フォームの生成
