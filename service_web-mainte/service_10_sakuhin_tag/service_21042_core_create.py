import os
import json
import subprocess

from django.conf import settings
from django.core.files.base import ContentFile
from django.core.files.storage import default_storage

from admin_app.service.service_main import ServiceMain
from admin_app.dao.dao_m_core import DaoMCore
from admin_app.dao.dao_t_update_history import DaoTUpdateHistory
from admin_app.dto.dto_10_sakuhin_tag.dto_21041_core_list import DtoCoreList
from admin_app.dto.dto_10_sakuhin_tag.dto_21042_core_create import DtoCoreCreateForm

class CoreCreateService(ServiceMain):

    # コアマスタテーブル用DAO
    dao_m_core = DaoMCore()
    # 変更履歴用DAO
    dao_t_update_history = DaoTUpdateHistory()

    '''初期描画'''
    def initialize(self):

        # m_coreに存在するコアコードの最大値を取得
        new_core_code = self.dao_m_core.selectMaxCoreCode()
        # 最大値+1を0埋めする（新規コアコード）
        new_core_code = str(int(new_core_code)+1).zfill(6)

        # 領域用DTOを画面DTOに詰める
        return self.unpack({'form':{'core_code':new_core_code}})

    '''コアマスタ作成処理'''
    def createCoreData(self,request):

        # POST内容をフォームに設定
        form = DtoCoreCreateForm(request.POST.copy())
        # 入力チェック
        if form.is_valid():
            # 入力チェックOK
            # ユーザの姓名取得
            full_name = request.user.get_full_name()
            # Dao登録用entityを作成
            entity = self.createDtoForInsert(form,full_name)
            # コアマスタの登録
            try :
                self.dao_m_core.insertCore(entity)
            except DaoMCore.DuplicateCoreCodeException as e:
                # コアコード重複によりエラーの場合
                # m_coreに存在するコアコードの最大値を取得
                new_core_code = self.dao_m_core.selectMaxCoreCode()
                # 最大値+1を0埋めする（新規コアコード）
                new_core_code = str(int(new_core_code)+1).zfill(6)
                form.data['core_code'] = new_core_code
                return self.unpack({'is_error':True,'form':form.data,'insert_error_corecode':True})

            except DaoMCore.DuplicateCoreNameException as e:
                # コア名重複によりエラーの場合
                return self.unpack({'is_error':True,'form':form.data,'insert_error_corename':True})

            # 更新履歴の登録
            # (更新テーブル名,操作内容,変更対象,備考,更新者)を設定
            entity = ('m_core','追加',form.cleaned_data['core_name'],'',full_name)
            self.dao_t_update_history.insert(entity)

            return self.unpack({'is_error':False})

        else :
            # 入力チェックNG
            errors = json.loads(form.errors.as_json())

            return self.unpack({'is_error':True,'form':form.data,'errors':json.loads(form.errors.as_json())})

    '''DAO登録用にコアマスタエンティティを作成'''
    def createDtoForInsert(self,form,full_name):
        return {
            'core_code' : form.cleaned_data['core_code'], # コアコード
            'core_name' : form.cleaned_data['core_name'], # コア名
            'full_name' : full_name, # 作成者/更新者
            }