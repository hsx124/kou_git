...
HomeUpdateController.prototype.execute = function() {
    ...
    //オーバーレイ制御（初期表示時）
    if ( djangoObj.dto.banner.is_checked) {
...
HomeUpdateController.prototype.bindEvent = function() {
    ...
    $('.select-hakusho-button').click(function() {
        self.selectHakusho(djangoObj.dto.whitepaper);
    });
    ...
    modelArray.push(
        GridUtil.createColModel({
            ...
            formatter: function(val, opt, rdt) {
                return "<button type='button' class='grid-btn-update' data-document-name='" + rdt.file_name + "' id-num='" + rdt.id + "'>" + '選択する' + '</button>';
            },
            sortable: false
        })
    );
