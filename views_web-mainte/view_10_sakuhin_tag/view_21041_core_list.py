...
from admin_app.service.service_10_sakuhin_tag.service_21041_core_list import CoreListService
...
class CoreListView(ListView):
    ...
    #テンプレート
    template_name = 'admin_app/10_sakuhin_tag/21041_core_list.html'
    ...
    def init(self, request):
        ...
        context = {}
        # サービスの実行
        dto = self.coreListService.initialize('m_core')
    ...
    def post(self, request):
        # コアコード・コア名を取得
        core_code = request.POST.get('delete-core-code')
        core_name = request.POST.get('delete-core-name')

        # 更新ユーザの姓名取得
        full_name = request.user.get_full_name()

        #サービスの実行
        dto = self.coreListService.deleteCoreData(core_code, core_name, full_name)
    ...
    def ajax(self, request):
        ...
        # サービスの実行
        # コア一覧検索時
        if 'core' in request.GET:
            dto = self.coreListService.getCoreList()
    ...
    def csvdownload(request):
        ...
        csv_data = coreListService.getCoreCsvData()
