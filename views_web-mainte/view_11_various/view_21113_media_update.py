...
from admin_app.service.service_11_various.service_21113_media_update import MediaUpdateService

class MediaUpdateView(MainView):

    # サービスクラス
    mediaUpdateService = MediaUpdateService()
    # テンプレート
    template_name = 'admin_app/11_various/21113_media_update.html'
    ...
    def get(self, request):
        ...
        # パラメータ名が正しい場合
        if 'mediac' in request.GET:
            media_code = request.GET['mediac']

            # サービスの実行
            dto = self.mediaUpdateService.initialize(media_code)
    ...
    def post(self, request):
        ...
        #サービスの実行
        dto = self.mediaUpdateService.updateMediaData(request)
