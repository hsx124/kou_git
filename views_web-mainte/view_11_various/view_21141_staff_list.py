...
from admin_app.service.service_11_various.service_21141_staff_list import StaffListService
...
class StaffListView(ListView):
    ...
    #テンプレート
    template_name = 'admin_app/11_various/21141_staff_list.html'
    ...
    def init(self, request):
        ...
        context = {}
        # サービスの実行
        dto = self.staffListService.initialize('m_staff')
    ...
    def post(self, request):
        # スタッフコード・スタッフ名を取得
        staff_code = request.POST.get('delete-staff-code')
        staff_name = request.POST.get('delete-staff-name')

        # 更新ユーザの姓名取得
        full_name = request.user.get_full_name()

        #サービスの実行
        dto = self.staffListService.deleteStaffData(staff_code, staff_name, full_name)
    ...
    def ajax(self, request):
        ...
        if 'staff' in request.GET:
            dto = self.staffListService.getStaffList()
    ...
    def csvdownload(request):
        ...
        csv_data = staffListService.getStaffCsvData()
