...
from admin_app.service.service_11_various.service_21142_staff_create import StaffCreateService

class StaffCreateView(MainView):

    #サービスクラス
    staffCreateService = StaffCreateService()
    #テンプレート
    template_name = 'admin_app/11_various/21142_staff_create.html'
    ...
    def init(self,request):
        ...
        context = {}
        #サービスの実行
        dto = self.staffCreateService.initialize()
    ...
    def post(self, request):
        ...
        context ={}
        #サービスの実行
        dto = self.staffCreateService.createStaffData(request)
        # javascriptデータ連携用
        context['object_to_javascript'] = json.dumps(dto, default=self.json_serial)
        #組み込みタグ用
        context['object_to_html'] = dto
        ...
        # 「スタッフ情報 新規登録画面」へリダイレクト
        if 'save_more_one' in request.POST:
            return redirect('admin_app:21142_staff_create')
