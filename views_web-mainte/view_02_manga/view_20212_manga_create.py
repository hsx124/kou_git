...
from admin_app.service.service_02_manga.service_20212_manga_create import MangaCreateService

class MangaCreateView(MainView):

    #サービスクラス
    mangaCreateService = MangaCreateService()
    #テンプレート
    template_name = 'admin_app/02_manga/20212_manga_create.html'
    ...
    def init(self,request):
        ...
        context = {}
        #サービスの実行
        dto = self.mangaCreateService.initialize()
    ...
    def post(self, request):
        ...
        context ={}
        #サービスの実行
        dto = self.mangaCreateService.createMangaData(request)
        # javascriptデータ連携用
        context['object_to_javascript'] = json.dumps(dto, default=self.json_serial)
        #組み込みタグ用
        context['object_to_html'] = dto
        ...
        # 「マンガ情報 新規登録画面」へリダイレクト
        if 'save_more_one' in request.POST:
            return redirect('admin_app:20212_manga_create')
    ...
    def ajax(self,request):
        ...
        if 'media' == process_code:
            # 掲載媒体取得
            dto = self.mangaCreateService.searchMedia(request.GET.get('keyword'))
        elif 'publisher' == process_code:
            # 出版社取得
            dto = self.mangaCreateService.searchPublisher(request.GET.get('keyword'))
        elif 'staff_role' == process_code:
            # スタッフ役割取得
            dto = self.mangaCreateService.searchStaffRole(request.GET.get('keyword'))
        elif 'staff' == process_code:
            # スタッフ役割取得
            dto = self.mangaCreateService.searchStaff(request.GET.get('keyword'))
