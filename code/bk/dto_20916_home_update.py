    ...
    def clean(self):
        ...
        if cleaned_data['url_checkbox']:
            if 'banner_url' in cleaned_data and cleaned_data['banner_url'] in validators.EMPTY_VALUES:
                self.add_error('banner_url',ValidationError(ValidationUtil.E0000022.format('遷移先','外部サイト'),code='required'))
        else:
...
class DtoHomeupdate(NamedTuple):
    ...
    class DtoBanner(NamedTuple):
        ...
        details :str
        external_site :str
        external_site_flg :str
        media_report_code : str
        media_report_name : str

    class DtoTMediaReport(NamedTuple):
        '''白書'''
        media_report_code : str
        media_report_name : str
        media_report_category :str
        details : str
        year : str
        update_user : str
        update_date : str
